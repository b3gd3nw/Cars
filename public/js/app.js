...
if (add_car) {
  add_car.addEventListener('click', function (e) {
    var target = add_car.getAttribute('data-path');
    axios__WEBPACK_IMPORTED_MODULE_0___default().get(target).then(function (responce) {
...
var edits_car = document.querySelectorAll('.edit_car');

if (edits_car) {
  edits_car.forEach(function (edit_car) {
    edit_car.addEventListener('click', function (e) {
      var target = edit_car.getAttribute('data-path');
      axios__WEBPACK_IMPORTED_MODULE_0___default().get(target).then(function (responce) {
        document.querySelector('.modal-card-body').innerHTML = responce.data.view;
